...
export const citizenAPI = {
  ...
  // Get pending connection requests (sent by citizen)
  async getPendingConnectionRequests(params = {}) {
    try {
      const queryString = new URLSearchParams(params).toString();
      const response = await apiClient.get(`/citizens/pending-connection-requests?${queryString}`);
      return {
        success: true,
        data: response.data,
      };
  ...
  async revokeConnectionRequest(connectionId) {
    try {
      const response = await apiClient.delete(`/citizens/revoke-connection-request/${connectionId}`);
      return {
        success: true,
        message: response.message,
      };
  ...
  // Revoke case-specific request
  async revokeCaseRequest(caseType, caseId, lawyerId) {
    try {
      const response = await apiClient.delete(`/citizens/revoke-case-request/${caseType}/${caseId}/${lawyerId}`);
      return {
        success: true,
        message: response.message,
      };
